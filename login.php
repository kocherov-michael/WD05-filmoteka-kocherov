...
if(isset($_POST['enter']))
{
    ...
    // Вытаскиваем из БД запись, у которой логин равняеться введенному, получаем из неё пароль и доступ
    $query = mysqli_query($link,"SELECT password, access FROM users WHERE login='".mysqli_real_escape_string($link, $_POST['login'])."' LIMIT 1");
